...
function cookieHeader() { return [...jar].map(([name, value]) => `${name}=${value}`).join('; '); }
function setCookies(value) { jar.clear(); storeCookies(String(value ?? '').split(';').map(x => x.trim()).filter(Boolean)); }
...
async function call(pathname, { method = 'GET', body, headers } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = https.request(HOST + pathname, { method, timeout: 30000, headers: portalHeaders({ ...(data ? { 'Content-Type': 'application/json', 'Content-Length': data.length } : {}), ...headers }) }, res => {
...
async function captcha() {
  const result = await call('/api/captcha'); const value = parse(result);
...
async function authenticate(input) {
  const result = await call('/api/security-taxpayer/authenticate', { method: 'POST', body: { username: input.username, password: input.password, ckey: input.ckey, cvalue: input.captcha } }); const value = parse(result);
  if (result.status < 200 || result.status >= 300 || !value?.token) throw new Error(value?.message || value?.error || `TCT trả HTTP ${result.status}.`);
  return value.token;
}
async function request(token, route, action) {
  const result = await call('/api' + route, { headers: { Authorization: `Bearer ${token}`, Action: encodeURIComponent(action), 'End-Point': END_POINT } });
