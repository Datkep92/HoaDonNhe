...
function safeMst(mst) { return /^(\d{10}(?:-\d{3})?|\d{13})$/.test(String(mst)); }
...
function saveAccount(input) {
  ...
  if (!safeMst(mst)) throw new Error('MST phải gồm 10, 13 số hoặc dạng 10 số-3 số.');
...
async function addOrLogin(mst) {
  if (!safeMst(mst)) throw new Error('Nhập MST gồm 10, 13 số hoặc dạng 10 số-3 số.');
  ensureIdle(); loginChallenge = null; authAccount = null;
